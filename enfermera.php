<?php session_start(); if(!isset($_SESSION[ 'usuario'])) header( 'Location: index.php'); else{ require_once( 'config.php'); $tipo=pg_query($cnn, "SELECT tipousuario FROM usuario WHERE nombreusuario = '$_SESSION[usuario]'"); pg_close($cnn); $usuario=pg_fetch_assoc($tipo); if($usuario[ 'tipousuario'] !='Enfermera' ) header(sprintf( 'Location: %s.php', strtolower($usuario[ 'tipousuario']))); } ?>
<!DOCTYPE html>
<html lang="es">
